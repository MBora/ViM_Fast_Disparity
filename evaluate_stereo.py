...
##  
        print('Validation sintel EPE: %.3f, D1: %.4f' % (
        mean_epe, mean_d1))
    else:
        print(f'Sintel Mean FPS:{np.array(fps[1:]).mean()}, MIN: {np.array(fps[1:]).min()}, MAX: {np.array(fps[1:]).max()}')
...
#   
    ...
    if eval_fps is not None:
        print(f'Things Mean FPS:{np.array(fps[1:]).mean()}, MIN: {np.array(fps[1:]).min()}, MAX: {np.array(fps[1:]).max()}')
...
def validate_kitti15(model,
    ...
    for i, sample in enumerate(val_dataset):
        ...
        with torch.no_grad():
            ...
            fps.append(1/(end-start))
            #mem.append(torch.cuda.max_memory_allocated(device=None))
...
#
    ...
    if eval_fps is not None:
        print(f'KITTI15 Mean FPS:{np.array(fps[1:]).mean()}, MIN: {np.array(fps[1:]).min()}, MAX: {np.array(fps[1:]).max()}')
    #print(mem)
...
#
    ...
    if eval_fps is None:
        ...
        print('Validation vKITTI EPE: %.3f, D1: %.4f' % (
       mean_epe, mean_d1))
    if eval_fps is not None:
        print(f'VKITTI2 Mean FPS:{np.array(fps[1:]).mean()}, MIN: {np.array(fps[1:]).min()}, MAX: {np.array(fps[1:]).max()}')
...
def inference_stereo(model,
    ...
    fps=[]
    mem=[]
    # calculate memory before training
